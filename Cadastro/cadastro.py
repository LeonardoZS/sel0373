...
def known_face_encodings(directory):
    for filename in os.listdir(directory):        
        if filename not in encodings_names:
            #image = cv2.imread("cadastro/"+filename)
#            image = cv2.resize(image, (800, 600))
#            cv2.imwrite("cadastro/"+filename, image)
...
#            # Compare the value of a specific field to a value in an if statement
            if token == 1:
                os.system(f"rm -r ~/Porteiro_eletronico/cadastro") 
                download_images()
                ...
                word=filename.split()
                filename ="\ ".join(word)
                os.system(f"rm -r ~/Porteiro_eletronico/cadastro/{filename}*")
        ...
        if contador_exclusão == 100:
            ...
            for file in os.listdir(os.curdir):
                if file.endswith(".jpg"):
                    os.system(f"rm ~/Porteiro_eletronico/{file}")
        contador_exclusão+=1
